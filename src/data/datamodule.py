...
from src.data.dataset import ChestXrayDataset, skip_failed_samples
...
class ChestXrayDataModule:
    ...
    def __init__(self, csv_path, batch_size=32, num_workers=4, image_size=224, label_smoothing=0.1):
        ...
        self.val_transform = None  #không thêm test_transform vì test tương tự val
    ...
    def setup(self):
        ...
        self.val_transform = get_val_transform(self.image_size)

    def train_dataloader(self):
        return DataLoader(
            ChestXrayDataset(self.train_df, self.train_transform, label_smoothing=self.label_smoothing), #label_smoothing=0.0 
            ...
            collate_fn = skip_failed_samples,
            #vì máy ko có GPU nên pin_memory=False để tránh lỗi, nếu có GPU thì pin_memory=True để tăng tốc độ truyền dữ liệu
        )

    def val_dataloader(self):
        return DataLoader(
            ChestXrayDataset(self.val_df, self.val_transform, label_smoothing=0.0), #val giữ nhãn cứng
            batch_size=self.batch_size,
    ...
    def test_dataloader(self):
        return DataLoader(
            ChestXrayDataset(self.test_df, self.test_transform, label_smoothing=0.0), #test không cần label smoothing
            batch_size=self.batch_size,
